...
setup_minio() {
  ...
  DATA_DIR1="$MINIO_BASE_DIR/data1"
  DATA_DIR2="$MINIO_BASE_DIR/data2"
  DATA_DIR3="$MINIO_BASE_DIR/data3"
  DATA_DIR4="$MINIO_BASE_DIR/data4"

  echo "Creating directories for distributed setup..."
  mkdir -p "$CERTS_DIR" || error_exit "Failed to create certificates directory."
  mkdir -p "$DATA_DIR1" "$DATA_DIR2" "$DATA_DIR3" "$DATA_DIR4" || error_exit "Failed to create data directories."
...
services:
  minio:
    ...
    environment:
      ...
      # Enable distributed mode with parity of 2
...
EOF
  ...
  echo "Setting up lifecycle rule..."
  ...
  echo "Adding lifecycle rule to bucket..."
  docker exec minio mc ilm add local/${BUCKET_NAME} --expire-days 1 --insecure

  echo "MinIO distributed setup completed with lifecycle rule configured for bucket '${BUCKET_NAME}'."
}
